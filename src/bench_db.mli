...
(** A bench db is a local file where we store bench results as we gather them
    ...
    that we have already run too many times unintentionally. *)

type t

(** Load an existing [t] at given filename, or initialize an empty db if not
    present. *)
...
(** Reload the db, add an entry overwriting any previous one if any already
    exists with [bench.key], and save to disk. *)
