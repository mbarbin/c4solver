...
module Mean = struct
  ...
  let zero = { span = Mtime.Span.zero; number_of_positions = 0; k_pos_per_s = 0 }
end
...
let mean ts =
  ...
  then Mean.zero
    ...
    let span = ref Mtime.Span.zero in
    let number_of_positions = ref 0 in
    let count = ref 0 in
    List.iter ts ~f:(fun t ->
      span := Mtime.Span.add !span t.span;
      number_of_positions := !number_of_positions + t.number_of_positions;
      count := !count + 1);
    let span = Mtime_extended.Span.divide !span ~by:!count in
    ...
    ; number_of_positions
    ; k_pos_per_s =
        (let ms = Mtime_extended.Span.to_ms span in
         if Float.equal ms 0.
         then 0
