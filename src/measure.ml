...
module Mean = struct
  ...
  let zero = { span = Time.Span.zero; number_of_positions = 0; k_pos_per_s = 0 }
end
...
let mean ts =
  ...
  then Mean.zero
    ...
    let span = ref Time.Span.zero in
    ...
    ; number_of_positions
    ; k_pos_per_s =
        (if Float.equal (Time.Span.to_ms span) 0.
        then 0
        else int_of_float (float_of_int number_of_positions /. Time.Span.to_ms span))
    })
;;
