...
let test_line line =
  ...
  let eval (module P : Position.S) =
    ...
    let can_play =
      List.init width ~f:(fun column -> column, P.can_play position ~column)
    in
    let is_winning =
  ...
  in
  let e1 = eval (module Position.Basic) in
  let e2 = eval (module Position.Bitboard) in
  let t =
    if equal e1 e2
    then e1
    else (
